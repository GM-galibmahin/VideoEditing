...
4. **Hero parallax frames** — Floating bordered "frames" with timecodes drift opposite to mouse movement (`data-depth` controls intensity). Hidden on mobile. Original positions/depths (ff1=30, ff2=55, ff3=80), now with z-index so the far frame `ff3` (depth 80) sits BEHIND the hero photo and `ff1`/`ff2` float IN FRONT of it — frames surround the cutout.
5. **Hero photo** — Plain static cutout at `./assets/me.png` (`.hero-photo`), NO card/border/glow/overlay/label by request. Sized by height (`min(72vh,640px)`, natural ratio), absolutely positioned center-right (`right:12%`) with a soft drop-shadow. The frames are positioned around it: `ff3` BEHIND the upper-left (z-index 1), `ff1` in front lower-right, `ff2` in front at waist-left — none may cover the face. The image never moves — only the frames parallax. On mobile it goes static, ~220px, centered above the heading (frames hidden). `me.png` must be a TIGHT cutout (transparent padding trimmed — a padded canvas makes it render tiny); current source is 653×1300 trimmed from the original export.
