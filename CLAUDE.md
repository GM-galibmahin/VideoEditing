...
- Responsive tiers (all in `style.css` bottom): ≤380px flip/fold (smaller type, single-col stats, hidden sec-notes); ≤820px mobile (hamburger, single-col about, frames+photo hidden, smaller scrubber); 821–1180px tablet (smaller hero photo, `ff2` hidden to avoid headline collision); ≤560px height landscape guard (hero min-height auto); ≥1800px 1440p/ultrawide goes FLUID — sections lose max-width and use 4vw side padding so content fills any screen; caps remain only where stretching hurts (`.reel` max `min(2200px,140vh)` so the player fits the viewport, `.ba-wrap` `min(1500px,120vh)`, `.about-wrap` 1700px for readable text measure); type/photo/frames scale up (h1 `11.5vw` to 330px, photo to 1100px); ≥2600px 4K bumps further (h1 to 380px, photo capped 1300px = source height). When changing the hero, re-check the headline-vs-photo gap at 1024, 1366, 1920, 2560 and 3840 (measure h1 span rects vs .hero-photo rect)
