...
5. **Hero photo** — Plain static cutout at `./assets/me.png` (`.hero-photo`), NO card/border/glow/overlay/label by request. ~450px clamped, 4:5 `object-fit:cover` crop, absolutely positioned center-right of the hero with the parallax frames around it. The image never moves — only the frames parallax. On mobile it goes static, ~220px, centered above the heading (frames stay hidden). Keep `me.png` web-sized (≤~900px, it's displayed ~450px) — a full-res export will bloat load.
